...
document.addEventListener("DOMContentLoaded", function (event) {
	...
	fetch(`webresources/users/getDepartment/${getCookie("username")}`) //aside buttons depend of user's department 
		...
		.then(result => departmentButtons()) //waiting for clicked department buttons
	...
	//receive JSON depends on radio buttons
	...
	let department = function (json) {
		departmentId = parseInt(json);
		urlDepartment = departmentId;
		switch (departmentId) {
			case 1:
				activeAll();
				break;
